...
def main(infer_dir: str, eval_dir: str):
    ...
    analyze_res = df.groupby(["task", "metric"], as_index=False)["score"].sum()/len(eval_res_list)
    overall_res = df.groupby("metric", as_index=False)["score"].sum()/len(eval_res_list)*len(tasks)
